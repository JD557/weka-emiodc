...
extends ClassifierTree {
	 ...
	 */
	public void buildClassifier(Instances data, int depth) 
			throws Exception {
	...
	protected ClassifierTree getNewTree(Instances train, Instances test, int depth) 
		...
		PruneableClassifierTree newTree = 
				new PruneableClassifierTree(m_toSelectModel, pruneTheTree, numSets, m_cleanup,
						m_seed);
		newTree.buildTree(train, test, !m_cleanup, depth-1);
		return newTree;
	}
	...
	private double errorsForLeaf() throws Exception {
		...
		for (int i=0;i<m_test.length;++i) {
			errors+=m_test[i].total()-
					m_test[i].perClass(localModel().distributions()[i].maxClass());
		}
		return errors;
	}
