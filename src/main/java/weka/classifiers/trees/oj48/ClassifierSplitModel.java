...
implements Cloneable, Serializable, RevisionHandler {
	...
	protected Distribution[] m_replicaDistribution;
	
	/** Active Splits */
	protected boolean[] m_activeSplit;
	...
	public final Distribution[] distributions() {
		return m_replicaDistribution;
	}
	
	public final boolean[] activeSplits() {
	...
	public final String dumpLabel(int index,Instances data) throws Exception {
		...
		int replicas = DataReplicator.getNumReplicas(data);
		...
		for (int i=0;i<replicas;++i) {
			int classValue=0;
			if (m_replicaDistribution.length>1) { // Numeric Values
				classValue=m_replicaDistribution[i].maxClass(index);
			}
			text.append(((Instances)data).classAttribute().
				value(classValue));
			if (i<replicas-1) {text.append(", ");}
		}
		text.append("}");
	...
	public void resetDistribution(Instances data) throws Exception {
		...
		if (m_replicaDistribution==null) {
			m_replicaDistribution = new Distribution[replicas.length];
		}
		else {
			m_replicaDistribution = new Distribution[m_replicaDistribution.length];
		}
		for (int i=0;i<m_replicaDistribution.length;++i) {
			m_replicaDistribution[i]=new Distribution(replicas[i],this);
		}
	}
