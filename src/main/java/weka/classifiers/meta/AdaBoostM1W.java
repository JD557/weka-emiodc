...
/**
 <!-- globalinfo-start -->
 * Class for boosting a nominal class classifier using the Adaboost.M1W method. Only nominal class problems can be tackled. Often dramatically improves performance, but sometimes overfits.<br/>
 * Based on Weka's AdaboostM1 implementation.
 * <br/>
 * For more information, see<br/>
 * <br/>
 * Günther Eibl and Karl Peter Pfeiffer. How to make adaboost.M1 work for weak base classifiers by changing only one line of the code.
 * <p/>
 <!-- globalinfo-end -->
 *
   ...
   <!-- options-end -->
	...
	protected void buildClassifierWithWeights(Instances data) 
		...
		for (m_NumIterationsPerformed = 0; m_NumIterationsPerformed < m_Classifiers.length; 
			...
			if (Utils.grOrEq(epsilon, 1.0 - 1.0/numClasses) || Utils.eq(epsilon, 0)) {
				if (m_NumIterationsPerformed == 0) {
					m_NumIterationsPerformed = 1; // If we're the first we have to to use it
				}
				break;
			...
			// Determine the weight to assign to this model
			reweight = (numClasses-1)*(1 - epsilon) / epsilon;
	...
	public static void main(String [] argv) {
		runClassifier(new AdaBoostM1W(), argv);
	}
}
